...
export const SplashScreen: React.FC<SplashScreenProps> = ({ onComplete }) => {
  ...
  const startTimers = useCallback(() => {
    ...
    timersRef.current = [...timers, finishTimer];
  ...
  useEffect(() => {
    ...
    return () => clearTimeout(fallbackTimer);
  }, [reduceMotion, onComplete, startTimers]);
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-0" aria-hidden>
        <motion.div
          ...
          animate={logoLoaded && phase >= 1 ? { scale: 1, opacity: 1 } : { scale: 0.82, opacity: 0 }}
        ...
        >
          ...
          <div className="relative flex h-32 w-32 items-center justify-center sm:h-40 sm:w-40">
            {!logoError ? (
              ...
              <div className="flex h-24 w-24 items-center justify-center rounded-full bg-slate-100 dark:bg-slate-800 sm:h-32 sm:w-32">
                <span className="text-lg font-bold text-slate-400 dark:text-slate-500">SSC</span>
              </div>
            )}
          </div>
